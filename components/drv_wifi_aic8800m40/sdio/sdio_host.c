 ...
 ****************************************************************************************
 */
void aicwf_sdio_host_tx_cfm_handler(struct sdio_host_env_tag *env, uint32_t *data)
{
    uint32_t queue_idx = 0; // data[0];
    ...
    // while (1)
    {
        ...
        if (host_id == 0)
        {
            ...
            printf("ERROR:No more confirmations\r\n");
            // break;
        }
        // set the cfm status
        ...
        // printf("sdio_host_tx_cfm_handler:used_idx=%d, 0x%p, status=%x\r\n",used_idx, env->pthis, txhdr->hw_hdr.cfm.status.value);
...
int aicwf_rwnx_sdio_platform_init(struct aic_sdio_dev *sdiodev)
{
    ...
    rwnx_plat = calloc(1, sizeof(struct rwnx_plat));
